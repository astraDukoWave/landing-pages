      ...
      <div className="shell">
        ...
        <div aria-label={menuCopy.filterLabel} role="group" className="mt-8 flex flex-wrap gap-2">
          {[{ id: 'all', label: menuCopy.all }, ...menuCategories].map(item => (
            <button key={item.id} type="button" aria-pressed={category === item.id} onClick={() => setCategory(item.id)} className={`min-h-11 rounded-full border px-4 py-2 text-sm transition-colors ${category === item.id ? 'border-brand-primary bg-brand-primary text-brand-onPrimary' : 'border-ink/25 text-ink hover:border-brand-primary'}`}>{item.label}</button>
          ))}
        </div>
          ...
          {visible.map(item => (
            <article key={item.id} className="border-b border-ink/20 py-7">
              <div className="flex flex-wrap items-baseline justify-between gap-2">
                ...
                <span className="text-xs text-brand-accent">{item.price !== undefined ? new Intl.NumberFormat('es-MX', { style: 'currency', currency: 'MXN' }).format(item.price) : demoCopy.pricePending}</span>
              </div>

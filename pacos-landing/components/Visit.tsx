    ...
    <section id="visitanos" className="border-t border-ink/15 py-16 md:py-24">
      <div className="shell grid gap-12 md:grid-cols-2">
        ...
        <div className="space-y-7">
          <div><h3 className="eyebrow">{visitCopy.address}</h3><address className="mt-3 text-xl not-italic">{business.address.street}<br />{business.address.neighborhood && <>{business.address.neighborhood}, </>}{business.address.postalCode} {business.address.city}, {business.address.region}</address><a href={mapsHref} target="_blank" rel="noopener noreferrer" className="mt-4 inline-flex min-h-11 items-center gap-2 text-sm text-brand-accent underline underline-offset-4">{visitCopy.maps} <span aria-hidden="true">↗</span></a></div>

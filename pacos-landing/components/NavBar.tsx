    ...
    <header className="sticky top-0 z-40 border-b border-ink/15 bg-surface/95 backdrop-blur-md">
      <nav aria-label={navCopy.label} className="shell flex min-h-20 items-center justify-between gap-3 py-3">
        <a href="#inicio" aria-label={navCopy.home} className="shrink-0">
          {business.logoSrc ? <Image src={business.logoSrc} alt={business.name} width={150} height={150} priority className="h-16 w-16 rounded-full object-contain" /> : <>
          <span className="block font-display text-3xl leading-none tracking-wide text-brand-accent sm:text-4xl">{business.shortName}</span>
        ...
        <div className="flex items-center gap-3 text-xs font-semibold sm:gap-7 sm:text-sm">
          <a className="py-3 hover:text-brand-accent" href="#menu">{navCopy.menu}</a>
          <a className="hidden py-3 hover:text-brand-accent sm:block" href="#visitanos">{navCopy.visit}</a>

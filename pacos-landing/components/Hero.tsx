    ...
    <section id="inicio" className="shell grid gap-12 py-12 md:grid-cols-[1.15fr_1fr] md:items-center md:gap-10 md:py-20">
      <div>
        ...
        <h1 className="mt-7 font-display text-[clamp(4.8rem,10vw,9rem)] leading-[0.87] tracking-tight">
          {heroCopy.headline.map((line, i) => <span key={line} className={`block ${i === 1 ? 'text-brand-primary' : ''}`}>{line}</span>)}
        </h1>
      ...
      <div className="relative border border-brand-primary/50 bg-brand-primary p-7 text-surface sm:p-10 md:rotate-2">
        <div className="flex justify-between border-b border-surface/40 pb-5 text-xs font-bold uppercase tracking-[0.15em]">
        ...
        <div className="flex items-center justify-between border-t border-surface/40 pt-5">

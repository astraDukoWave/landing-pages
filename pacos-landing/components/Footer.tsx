  ...
  return (
    <footer className="border-t border-ink/20 py-9">
      <div className="shell flex flex-wrap items-center justify-between gap-6">
        <div>{business.logoSrc ? <Image src={business.logoSrc} alt={business.name} width={150} height={150} className="h-20 w-20 rounded-full object-contain" /> : <p className="font-display text-3xl text-brand-accent">{business.shortName}</p>}<p className="mt-2 text-xs text-ink-muted">© {copyrightYear} · {business.demo ? footerCopy.demo : business.name}</p></div>
